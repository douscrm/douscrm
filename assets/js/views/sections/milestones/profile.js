...
class MilestoneProfile extends Component {
	constructor(props) {
		super(props);
	...
	componentDidMount() {
		...
		axios.get(`${globals.api}/milestones/${id}`).then((response) => {
	...
	deleteItem() {
		...
		axios.delete(`${globals.api}/milestones/${id}`).then((response) => {
			...
			<div className="row">
				<div className="col-12">
					<BoxCard>
						...
						<BoxCardFooter>
							<Link className="btn btn-primary float-left" to={`/milestones/${this.model.id}/update`}>Edit milestone</Link>
