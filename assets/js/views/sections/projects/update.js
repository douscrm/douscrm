...
class ProjectCreate extends Component {
	...
	componentDidMount() {
		...
		axios.get(`${globals.api}/projects/${id}`, {
			name: this.name
		}).then((response) => {
	...
	onSubmit(e) {
		...
		axios.patch(`${globals.api}/projects/${self.model.id}`, self.model).then((response) => {
				...
				<div className="col-12">
					<nav aria-label="breadcrumb" role="navigation">
						<ol className="breadcrumb">
							...
							<li className="breadcrumb-item">
								<Link to={`/projects/${this.model.id}`}>{this.model.name}</Link>
							</li>
