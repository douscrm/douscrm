...
class OportunityProfile extends Component {
	constructor(props) {
		super(props);
	...
	componentDidMount() {
		...
		axios.get(`${globals.api}/oportunities/${id}`).then((response) => {
	...
	deleteItem() {
		...
		axios.delete(`${globals.api}/oportunities/${id}`).then((response) => {
			...
			<div className="row">
				<div className="col-12">
					<BoxCard>
						...
						<BoxCardFooter>
							<Link className="btn btn-primary float-left" to={`/oportunities/${this.model.id}/update`}>Edit oportunity</Link>
...
export default OportunityProfile;